...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = await Prismic.api('https://prismic-nuxtjs-test.prismic.io/api/v2', Object.assign({}, options,  {}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://prismic-nuxtjs-test.prismic.io/api/v2'
      },
      predicates() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady((app) => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=prismic-nuxtjs-test&new=true'
  ...
  if (process.client && process.static && route.path !== '/preview') {
    ...
    const repo = 'prismic-nuxtjs-test'
